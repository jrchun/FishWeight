...
data.all <- read.table('fish.txt', header = F)
names(data.all) <- c('Obs', 'Species', 'Weight', 'Length1', 'Length2',
                  'Length3', 'Height', 'Width', 'Sex')
data.all$Species <- as.factor(data.all$Species) #Species : 종 이므로 팩터로 변경
